...
**If you're editing this project:** change extension behavior in `src/` only. The `dist/` folder is rebuilt from scratch every time you run `build.sh` and is not committed — anything you edit there is silently thrown away on the next build. If you need to change a permission or version number, edit it in *both* `firefox/manifest.json` and `chrome/manifest.json`.
